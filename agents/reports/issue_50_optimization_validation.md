...
|---|---:|---:|---|
| Full process wall, original reported series | 3.80 s | 3.46 s | Indicative only; build and sampling conditions were not fully recorded, so the 8.9% ratio is not a controlled speed claim |
| Peak sampled whole-device VRAM | 6,033 MiB in the original baseline sample | 3,537 MiB maximum | A later 50 ms matched audit caught a 7,073 MiB baseline transient in five of ten runs; the 3,584 MiB target was met only for this optimized 24-frame input |
...
The originally reported optimized full-process runs were 3.46, 3.49, and 3.45 s. A second series retained beside them under `/home/alex/MotionCorr-issue50-opt-performance/run1` through `run3` measured 3.56, 3.57, and 3.53 s; its GPU sampling rate differed from the reported series. Both series completed, and the second must not be silently excluded or pooled into a controlled comparison. A later matched audit under `/tmp/checker-perf-audit/` used a 50 ms sampler for both binaries and recorded ten optimized and ten baseline runs (means 3.503 s and 3.941 s); its script used no 8-core affinity cap and does not establish the Release-build or shared-host result. That audit caught the baseline at 7,073 MiB in five of ten runs. Its separate 10 ms warmups sampled only the optimized binary, so they do not establish a 10 ms baseline peak. A separate instrumented one-movie run recorded 2.35 s inside the movie timer; its process wall time is not compared to the full-process series. Stage timers are host intervals and may overlap CUDA work. The full-process output gap remains material; its substeps are not yet individually instrumented.
...
- **CPU Gate 2:** The recorded single-movie comparison fails on relative-image RMSE: `0.007130747` versus the `0.001` limit. Shift RMS `0.004159`, shift max `0.006441`, absolute image RMSE `0.005665`, and STAR metadata pass their respective checks. The reference binary path and hash were not recorded with that result, so it must not be presented as a verified upstream RELION comparison. Exact equality to the earlier CUDA output establishes that this optimization did not change that output.
...
The memory reduction and exact CUDA-to-CUDA output are measured; the precise full-process speed gain needs a controlled matched rerun. This is not a release-ready Gate 2 result. Full 24-movie **CUDA-to-CUDA** equality has been checked, while full-dataset CPU/RELION Gate 2, low-memory injection/CPU fallback, no-gain and defect variants, and larger-input VRAM headroom remain to be checked. A heuristic specification script reported conformance after the profiling tool was named explicitly in the design, but that script does not override these empirical gates.
