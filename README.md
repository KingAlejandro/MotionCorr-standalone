...
- **Full 24-Movie RELION SPA Tutorial Dataset on Linux (`4-gpu-vm`, AMD EPYC 7452, GCC 13.3.0)**: All 24 compressed TIFF movies (`20170629_00021_frameImage.tiff` through `20170629_00049_frameImage.tiff`) with gain correction, 5 × 5 patches, and dose weighting:
  - **Single-Thread Strict Parity (`--j 1`)**: Standalone MotionCorr and full RELION 5.1 (commit `ad0b230c`) produced **100% byte-identical pixel payloads** (Image RMSE = `0.000000e+00`, max pixel difference = `0.000000e+00`), **identical frame trajectories** (RMS shift error = `0.000000 px`), and **identical STAR metadata** across all 24 movies, passing the strict acceptance gate on 24/24 movies. Dataset-level `corrected_micrographs.star` matches with 0 discrepancies.
  - **Four-Thread Execution (`--j 4`)**: Achieved a **3.46x wall-clock speedup** (7m 40s vs 26m 31s for the full 24-movie dataset). Trajectory shift deviations due to OpenMP reduction non-associativity are strictly bounded (mean RMS shift 0.0033 px, max shift error 0.0158 px), well within the 0.05-pixel trajectory threshold.
  - Full details, per-movie tables, resource metrics, and manifests are documented in [`docs/spa_24_movies_validation.md`](docs/spa_24_movies_validation.md) and [`docs/spa_24_movies_manifest.json`](docs/spa_24_movies_manifest.json).

This establishes complete single-thread CPU numerical parity and multi-thread performance across both macOS ARM64 and Linux x86_64 for the full tutorial dataset.
