...
| C: transfer and timing | Measure upload, decoder, output and sync costs; optimize bounded upload staging only after an A/B integrated baseline | `tools/profile_cuda_movie.py` and a report under `agents/reports/`; changes to runner instrumentation are submitted to the integration owner |
