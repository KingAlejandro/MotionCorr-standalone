...
| C: transfer and timing | Measure upload, decoder, output and sync costs; optimize bounded upload staging only after an A/B integrated baseline | `tools/profile_cuda_movie.py` and `agents/reports/issue_50_optimization_validation.md`; changes to runner instrumentation are submitted to the integration owner |
