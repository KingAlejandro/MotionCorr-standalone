#!/usr/bin/env python3
"""Evaluate Metal output against CPU j1 across the RELION SPA dataset.
...
- Metal Global Alignment Profile (H2D, Custom Kernel, MPSGraph FFT, D2H, Total Metal ms, Peak VRAM)
- Full Movie Wall Time & Process RSS
"""
...
def parse_metal_profile(log_path: Path) -> Dict[str, Any]:
    ...
    patterns = {
        ...
        "mpsgraph_ms": r"(?:MPSGraph FFT|cuFFT)\s*execution time:\s*([\d\.]+)\s*ms",
        "d2h_ms": r"Device-to-Host transfer time:\s*([\d\.]+)\s*ms",
        "total_metal_ms": r"Total (?:Metal|GPU)\s*alignment time:\s*([\d\.]+)\s*ms",
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate Metal benchmark against CPU across dataset")
    parser.add_argument("--metal-dir", required=True, type=Path)
    ...
    parser.add_argument("--metal-run-log", required=True, type=Path,
                        help="Full Metal run log from /usr/bin/time -l or -v, including exit status")
    parser.add_argument("--expected-movies", type=int, default=24)
    parser.add_argument("--json-out", type=Path, default=None)
    args = parser.parse_args()

    if not args.star.is_file() or not args.metal_run_log.is_file():
        parser.error("--star and --metal-run-log must name existing files")
    ...
    results = []
    print(f"{'#':<3} {'Movie Name':<30} {'Metal ms':>8} {'Traj RMS':>10} {'Max Shift':>10} {'Img RMSE':>10} {'Rel RMSE':>10} {'Gate 2'}")
    print("-" * 88)

    for i, mov in enumerate(movie_names, 1):
        metal_mrc = args.metal_dir / "Movies" / f"{mov}.mrc"
        metal_star = args.metal_dir / "Movies" / f"{mov}.star"
        metal_log = args.metal_dir / "Movies" / f"{mov}.log"

        j1_mrc = args.cpu_j1_dir / "Movies" / f"{mov}.mrc"
        j1_star = args.cpu_j1_dir / "Movies" / f"{mov}.star"

        required = (metal_mrc, metal_star, j1_mrc, j1_star)
        ...
        profile = parse_metal_profile(metal_log)
        comp_j1 = run_comparison(j1_mrc, metal_mrc, j1_star, metal_star, args.metal_run_log)
        ...
        if args.cpu_j4_dir:
            ...
            comp_j4 = run_comparison(j4_mrc, metal_mrc, j4_star, metal_star, args.metal_run_log)
        ...
        metal_ms = profile.get("total_metal_ms")
        metal_text = f"{metal_ms:.1f}" if metal_ms is not None else "n/a"
        print(f"[{i:02d}] {mov:<30} {metal_text:>8} {fmt(traj_j1.get('coord_rms_error')):>10} {fmt(traj_j1.get('max_shift_error')):>10} {fmt(img_j1.get('rmse')):>10} {fmt(img_j1.get('relative_rmse')):>10} {gate_str:>6}")
